...
- Kein Volltext gelesen (Verlagsseiten, arXiv, doi.org gesperrt). **Keine Seiten- oder
  ...
  wo ein Kapitel im Treffer sichtbar war.
...
- POD-Galerkin: Projektion der Erhaltungsgleichungen (PDE) auf wenige Moden; intrusiv, braucht
  die Gleichungen. Trunkierung verliert die Rückwirkung der abgeschnittenen Moden, typisch als
  Drift des Mittelwerts (Shift-Mode) oder Instabilität.
...
- \edmd: Galerkin-Projektion des Koopman-Operators auf den Raum der Observablen (ch3 Z. 303–307);
  Trunkierung zeigt sich als Scheineigenwerte und Closure-Problem (ch3 Z. 308–312).
...
- Eigene Einordnung: \sindy{} mit Polynombibliothek auf POD-Koordinaten entspricht Operator
  Inference plus Sparsität; die Shift-Mode-Korrektur entspricht in ihrer Rolle dem
  Offset, den der Störgrößenbeobachter der Arbeit aufnimmt. Beides nicht belegt.

**Quellen**

| Quelle | Aussage (Herkunft) | Status | Stelle |
|---|---|---|---|
| `loiseauConstrainedSparseGalerkin2018` (Bib) | \sindy{} auf POD-Koordinaten mit Nebenbedingungen aus Erhaltungsgleichungen (ch3 Z. 445–447) | Bib | lokal |
...
| Rowley, C. W.; Dawson, S. T. M. (2017): Model Reduction for Flow Analysis and Control. Annu. Rev. Fluid Mech. 49, 387–417 (G) | Übersicht POD-Galerkin, balanced truncation, DMD, Koopman für Analyse und Regelung (S) | [nur bibliografisch] | lokal |
...
Ergänzend: Benner, Gugercin, Willcox 2015 (SIAM Review 57(4), 483–531, G) als Übersicht
projektionsbasierter Reduktion; Holmes, Lumley, Berkooz, Rowley 2012 (2. Aufl., Cambridge UP, G)
als Lehrbuch; Narayanan, Sicoli, Mehta 2026 (arXiv:2604.24646, G) koppeln ein
...
- Lücke: keine \sindy-Arbeit mit Messdaten und Bibliothek der Form Temperaturdifferenz ×
  Betriebspunktfaktor für Wärmeübertrager oder Motorthermik gefunden (auch Auftrag A, A3.3).
...
| Yahagi, S.; Yonezawa, A.; Seto, H.; Yonezawa, H.; Kajiwara, I. (2025): arXiv:2503.05154 (G), Titel v1 „Sparse identification of nonlinear dynamics with high accuracy and reliability under noisy conditions for applications to industrial systems“ | \sindyc{} am Diesel-Luftpfad mit Library-Bagging, begründet mit „physically motivated basis functions are not always available“; Simulation | [nur Abstract] | – |
...
„Biases“ in einem RK4-\sindy; Abdullah & Christofides 2023 (Übersicht Sparse-Identifikation und
MPC in der Prozesstechnik, Zeitschrift nur aus PII abgeleitet); Li, Larrañaga, Brunton, Fasel 2026
(arXiv:2607.15077, Tutorial, G). Der IJCAS-Artikel „Sparse Identification and Nonlinear Model
Predictive Control for Diesel Engine Air Path System“ (DOI 10.1007/s12555-024-0452-9, T) ist
vermutlich `yahagiSparseIdentificationNonlinear2025` (Bib), Zuordnung ungeprüft.
...
- Koopman-Seite: Wird der Reglerausgang als exogener Eingang behandelt, korreliert er mit dem
  Rauschen und der Bias steigt; Sollwert oder Vorsteuerung als Eingang senken ihn
  (Dahdah & Forbes 2024, Bib).
...
- Schema „Fit auf Open-Loop-Daten, geregelte Daten nur zur Validierung“ mit industriellen
  Messdaten: **nicht gefunden**. Die Arbeit begründet es deshalb selbst über die
  klassische Closed-Loop-Theorie (Forssell & Ljung 1999, Ljung 1999, Bib) und prüft es am
  Prüfstand gegen die bekannte Wahrheit (ch3 Z. 922–925).
...
| Quelle | Aussage (Herkunft) | Status | Stelle |
|---|---|---|---|
| `dahdahClosedloopKoopmanOperator2024` (Bib) | s. oben (S) | Bib | lokal |
...
| `lovlandClosedloopIdentificationChallenge2025` (Bib; arXiv:2509.01188, G) | Bias stationärer Sensitivitätsmodelle aus Closed-Loop-Daten in der Feedback-Optimierung; Divergenz, wenn eine Matrix aus Sensitivität und Rausch-Signal-Verhältnis negativ definit ist | [nur Abstract] | ch3 zitiert Theorem 1, Gl. 26 |
| `forssellClosedloopIdenticationRevisited1999`, `ljungSystemIdentificationTheory1999` (Bib) | Konsistenzbedingungen der direkten Methode | Bib | lokal |
| `strasserOverviewKoopmanbasedControl2026` (Bib; arXiv:2509.02839, G) | Übersicht Koopman-Regelung von Fehlerschranken bis zu Closed-Loop-Garantien (Titelebene) | Bib | lokal |
...
(T): Störsignal im Kreis, Online-\sindyc{} (S). „Real-time adaptive sparse-identification-based
predictive control of nonlinear processes“, Chem. Eng. Res. Des. (PII S0263876223004483, T):
Online-Update in LMPC (S). Grasev 2026 (arXiv:2604.01730, G): Koopman-MPC mit
Störgrößenbeobachter am Turbofan, Simulationsdaten `[nur Abstract]`.
...
**Ort in den Kapiteln:** ch3 Z. 625–628 („zu erwarten“ durch Belege ersetzen); ch3 Z. 749–754
(Koopman-Seite, Wu 2026 ergänzen); ch3 Z. 922–925 (Prüfung gegen bekannte Wahrheit als eigene
Antwort auf die Lücke).
...
- **Kalman ↔ exponentielle Glättung (eigene Herleitung, sympy-geprüft):** Störung als Random
  Walk $d_k=d_{k-1}+w_k$, Residuum $z_k=y_k-\hat y_k=d_k+v_k$ mit Varianzen $q$, $r$.
  Stationärer Kalman-Filter:
  $\hat d_k=\hat d_{k-1}+L\,(z_k-\hat d_{k-1})=(1-L)\,\hat d_{k-1}+L\,z_k$,
...
- Abstimmung: Bias-Varianz-Zielkonflikt eines Kalman-Störbeobachters bei unvollständigem
  Störmodell (Li u. a. 2024, Preprint).
- Bezug Erstgutachter: Die Kovarianzwahl $q/r$ ist die eine Stellschraube des
  Störgrößenbeobachters; sie entspricht direkt dem Glättungsfaktor.
...
| Pannocchia, G. (2015): Offset-Free Tracking MPC: A Tutorial Review and Comparison of Different Formulations. ECC 2015, Linz, 527–532 (G) | Vergleich der Formulierungen (Titelebene) | [nur bibliografisch] | lokal |
...
Ergänzend: Li, Shi, Lyu, Tang, Shi 2024 (arXiv:2410.05061, G) `[nur Abstract]`: Kalman-Störbeobachter
optimal bei bekanntem Störmodell, sonst Bias-Varianz-Zielkonflikt; Hyndman, Koehler, Ord, Snyder
2008 (Springer, G): Glättung im Zustandsraum; ein J.-Process-Control-Artikel 2026 zu offsetfreiem
MPC mit abstimmbarer Störschätzdynamik (PII S095915242600020X, Autoren unbekannt).
...
- Identifikation für die Regelung: Modellfehler müssen nicht klein, sondern für den Reglerentwurf
  passend sein; Modellbildung und Reglerentwurf sind iterativ (Skelton 1989). Schwerpunkt
  verschob sich zu regelungsorientierten Unsicherheitsmengen und Versuchsplanung (Gevers 2005,
  Hjalmarsson 2005).
- Verstärkung vor Einzelgenauigkeit: Richtung und Vorzeichen der Verstärkung beeinflussen die
  Regelgüte stärker als die Genauigkeit einzelner Elemente (Li & Lee 1996; Koung & MacGregor 1994;
  Benchmark Wärmeübertrager bei Jacobsen & Skogestad 1994). Für die integrale Regelbarkeit gibt es
  Bedingungen an die identifizierte Verstärkung (Darby & Nikolaou 2009).
...
- Anwendungsorientierte Versuchsplanung für MPC: Ebadat u. a. 2013 (Preprint).
...
Ergänzend: Koung & MacGregor 1994 (Automatica, PII 0005109894900949, T; Initialen widersprüchlich);
Jacobsen & Skogestad 1994 „Identification of ill-conditioned plants – a benchmark problem“
(DOI 10.1007/BFb0036269, T); Van den Hof & Schrama 1995 „Identification and Control –
Closed-loop Issues“ (Automatica 31(12), 1751–1770, S); Misra u. a. 2017 (Processes 5(3), 42, G);
Ebadat u. a. 2013 (arXiv:1303.5199, G); `bomboisLeastCostlyIdentification2006`,
`geversIdentificationInformationMatrix2009` (Bib).
...
- Hyperparameter (Schwellwert, Bibliothek, Ordnung, Liftung) nur auf Auswahldaten; Testdaten
  unberührt. Sonst liegt die Überanpassung der Auswahl in der Größenordnung der Unterschiede
  zwischen den Verfahren (Cawley & Talbot 2010; Varma & Simon 2006).
- Gepaarter Vergleich auf denselben Testabschnitten; Fehler sind seriell korreliert, daher
  Test nach Diebold & Mariano 1995 oder Block-Bootstrap (Künsch 1989). Diebold 2015: Der Test
  vergleicht Prognosen, nicht Modelle.
- Neuronale Netze einordnen: Feedforward-Netze sind NARX-Modelle (Ljung u. a. 2020); Vergleich
  mit NN ist daher ein Vergleich innerhalb der NARX-Familie (Bezug Zweitgutachter).
...
| Ljung, L.; Andersson, C.; Tiels, K.; Schön, T. B. (2020): Deep learning and system identification. IFAC-PapersOnLine 53(2), 1175–1181 (G) | Feedforward-Netze sind NARX-Modelle (S) | [nur bibliografisch] | lokal |
...
Ergänzend: Diebold 2015 (JBES 33(1), G); Schoukens & Noël 2017 (IFAC-PapersOnLine 50(1), 448–453, G);
Künsch 1989 und Efron & Tibshirani 1993 (BibTeX in E); Narayanan u. a. 2026 (Frage 1) als
Beispiel eines Vergleichs innerhalb und außerhalb der Trainingsbedingungen.

**Ort in den Kapiteln:** ch3 Z. 1161–1174 (Ausschluss neuronaler Netze: NARX-Argument ergänzen);
...
% Titel der Version 1; später geänderter Titel laut Treffer (T):
% "Sparse Identification of Nonlinear Dynamics Enhanced by Ensemble Learning, ..."
% Key-Kollision mit yahagiSparseIdentificationNonlinear2025 (Bib) -> Suffix a
...
% ungeprüft: journal = {Computers \& Chemical Engineering}; year = {2023} (aus PII abgeleitet)
@article{abdullahDatabasedModelingControl2023,
  author    = {Abdullah, Fahim and Christofides, P.},
...
@misc{grasevKoopmanbasedNonlinearIdentification2026,
  ...
  eprint        = {2604.01730},
  archivePrefix = {arXiv},
  note          = {Preprint; Titel v1 abweichend}
}
...
- Kein Volltext gelesen; alle Stellen „lokal nachschlagen“. Inhaltsaussagen auf Abstract-Ebene,
  meist (S).
- DOIs nur (S) oder abgeleitet: Hjalmarsson 2005, Pannocchia u. a. 2015 (IFAC), Callaham u. a.
  2022 (schwache G-Quelle). Keine DOI gefunden: Koung & MacGregor 1994, Li & Lee 1996,
  Van den Hof & Schrama 1995.
...
- Äquivalenz Kalman-Filter ↔ exponentielle Glättung ist eigene Herleitung (sympy-geprüft, Skript
  ...
  Rawlings/Mayne/Diehl lokal nachschlagen.
- Eigene Einordnungen (SINDy ≈ Operator Inference plus Sparsität; Shift-Mode ≈ Offset) nicht belegt.
- Titelvarianten: Loiseau 2018 (modelling/modeling), Schoukens & Ljung (Road Map/Roadmap),
  Grasev 2026 (v1/v2), Yahagi arXiv:2503.05154 (v1 vs. später).
- Keys: Kollision bei Yahagi 2025 (Suffix a); Zuordnung IJCAS-DOI und arXiv-IDs zu den Bib-Keys
  der Yahagi-/Yonezawa-Gruppe ungeprüft.
- Lücken: Frage 2 keine \sindy-Arbeit mit Messdaten und Wärmeübertrager-Bibliothek; Frage 3 kein
  Beleg für „Fit Open-Loop, Validierung Closed-Loop“ mit Industriedaten und keine
  \sindy-spezifische Closed-Loop-Bias-Analyse.
...
#   d_k = d_{k-1} + w_k,  w ~ N(0, q)      (Ausgangsstörung als Random Walk)
#   z_k = d_k + v_k,      v ~ N(0, r)      (z = Messung minus Modellvorhersage)
# Behauptung: Update d_k = d_{k-1} + L (z_k - d_{k-1}) ist exponentielle Glättung mit alpha = L.
import sympy as sp
